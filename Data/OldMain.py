...
def process_wallets():
    ...
    for wallet in wallets.copy():  # Iterate over a copy to safely modify the original list
        ...
        # Save to checked regardless of result
        with open('CheckedWallets.txt', 'a') as f:
            f.write(f"{wallet}\n")
        checked.add(wallet)
        
        # Remove from NewWallets
        ...
        if any(metric is None for metric in [winrate, roi, sol_balance]):
            print(f"{Fore.YELLOW}Failed to get complete data{Style.RESET_ALL}")
        ...
        status = [
            ...
            f"SOL Balance: {sol_balance:.2f} {'✅' if sol_ok else '❌'}"
        ]
        ...
        if all([winrate_ok, roi_ok, sol_ok]):
            ...
            print(f"{Fore.RED}⛔ Doesn't meet all criteria{Style.RESET_ALL}")
            disqualified_wallets += 1
